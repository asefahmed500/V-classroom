...
export default function RoomJoinPage({ params }: PageProps) {
  ...
  useEffect(() => {
    checkAuthAndFetchRoom()
  }, [params.id])

  const checkAuthAndFetchRoom = async () => {
    try {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      if (!isAuthenticated) {
        ...
        const signupUrl = `/auth/signup?redirect=${encodeURIComponent(`/rooms/${params.id}`)}&email=${encodeURIComponent(userInfo.email)}&name=${encodeURIComponent(userInfo.name)}`
          ...
          <div className="space-y-3">
            ...
            {!isAuthenticated && (
              <div className="text-center">
                ...
                <Link href={`/auth/login?redirect=${encodeURIComponent(`/rooms/${params.id}`)}`}>
